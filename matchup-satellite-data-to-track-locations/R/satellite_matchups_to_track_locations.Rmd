...
* A loggerhead turtle telemetry track that has been sub-sampled to reduce the data requests needed for this tutorial from over 1200 to 25. The turtle was raised in captivity in Japan, then tagged and released on 05/04/2005 in the Central Pacific. Its tag transmitted for over 3 years and went all the way to the Southern tip of Baja California. The track data can be downloaded here: https://sunny_change_this/25317_05_subsampled.dat

[COMMENTS] LINK TO THE DATA SOURCE?
...
```
## Import Turtle track data into a data frame

```{r tag}
turtle.df <- read.csv("25317_05_subsampled.dat")

print(head(turtle.df, 2))

print('Spatial corrdinate ranges')
paste('latitude range', round(min(turtle.df$mean_lat), 2), "," ,round(max(turtle.df$mean_lat), 2))
paste('longitude range', round(min(turtle.df$mean_lon), 2), "," ,round(max(turtle.df$mean_lon), 2))
...
xcoord <- turtle.df$mean_lon  
ycoord <- turtle.df$mean_lat
tcoord <- turtle.df$date



```

## Request Satellite dataset and download its metadata

We will use the SeaWiFS 8-day composite chlorophyll dataset (ID erdSW2018chla8day) using **rerddap**.
...
plotTrack(chl, xcoord, ycoord, tcoord, plotColor = 'algae')
