...
list.of.packages <- c("rerddap", "plotdap", "parsedate", "ggplot2", "rerddapXtracto",
                       "maps", "mapdata", "RColorBrewer","viridis")
...
ggplot(turtle_df, aes(mean_lon,mean_lat)) +
  geom_path(group=1)+
  geom_point(aes(x=mean_lon,y=mean_lat), shape=20, size=2, fill="black")+
...
Ideally we would use a daily dataset, selecting the day correspond to the dates of the days along the track. However, chlorophyll measurements can have lots of missing data, primarily due to cloud cover. To reduce data gaps and improve the likelihood of data availability for our matchups, we can use a monthly composite product. 
...
**rxtracto** computes statistics using all the pixels found in the search radius around each track point. Because chl-a concentration tends to have very high values and very low values, we will work with the median value around wach track point, **median chlor_a**
...
```
After the log transformation the data looks more normally distributed.
The range of log chlorophyll is about -2.9 to -0.3 but most of the values are between -2.5 and -0.8.
Knowing the distribution of the values can also help to set the color bar range for our map.


## Plotting the results using plotTrack
We will use the "plotTrack" function to plot the results.  "plotTrack" is a function of the "rerddapXtracto" package designed specifically to plot the results from "rxtracto".  However it's not very customizable.
